...
## Written by ManKind [Discord](https://discord.gg/39fNFepADG) for QB-Core / ESX
...
- # [qb-inventory]
    ...
    - Locate function `IsVehicleOwned` inside `qb-inventory/server/main.lua` and replace with:
        ...
        ```
        - **NOTE:** doing the above function replace will not update glovebox/trunk items when fake plate is added.
...
- # [ox_inventory] **Version 2.23.4+ Required**
    ...
    -   **NOTE:** INTEGRATION WILL BE KEPT UP TO DATE IN THIS FILE TO PREVENT HAVING TO UPDATE THE SCRIPT BECAUSE OF AN OX_INVENTORY UPDATE
...
- # [qb-core]
    ...
    - Locate RegisterNetEvent `qb-garages:client:takeOutGarage` inside `qb-garages/client/main.lua` and replace with:
        ...
        ```
        - **NOTE:** Not doing the above 2 function replaces will result in not receiving keys for a vehicle with a custom plate.
...
- `exports["mk_plates"]:HasFakePlate(NetId)` **[Server]** export you can use to check if a vehicle has a fake plate. 
    -   Example of use:
        ...
        ```
    - **NOTE:** You can use this export in any Server side file if you wish to add fake plate checks into any of your other scripts!
...
-   If you wish to add checks for fake plates in scripts that are already checking for plate numbers you will have to edit your MySQL queries.
    - Example: 
        ...
        --Replacement
        local Result = MySQL.query.await('SELECT * FROM player_vehicles WHERE (plate = :vehplate OR fakeplate = :vehplate)', {vehplate = plate})
        ```

    - **NOTE:** This allows the script to still locate your vehicle even if it has a fake plate equipped.
